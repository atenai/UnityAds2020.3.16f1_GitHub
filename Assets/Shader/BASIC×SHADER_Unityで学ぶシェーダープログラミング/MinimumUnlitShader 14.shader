//最小のシェーダーコードらしい
Shader "BASICxSHADER/Unlit/MinimumUnlitShader14" //Shader 直後の名前がマテリアルの「Shader」選択項目に反映されます。

{
...

	{
		...
		//Queue レンダリング順 (Background | Geometry | AlphaTest | Transparent | Overlay)
		//RenderType グループ分類 (Opaque | Transparent | TransparentCutout | Background | Overlay)
...

		{
			...
			//LightMode ライティング (Always | ForwardBase | ForwardAdd | Deferred | ShadowCaster | MotionVectors)
